...
Simulation.prototype._lastMessageMonth = -1;

Simulation.prototype.checkMonthlyMessage = function() {
  const date = this.getDate();
  if (date.month !== this._lastMessageMonth) {
    this._lastMessageMonth = date.month;

    // 메시지 발송 이벤트, subject는 메시지 구분용 문자열
    this._emitEvent(Messages.FRONT_END_MESSAGE, {subject: "ONE_MONTH_PASSED"});
  }
};
